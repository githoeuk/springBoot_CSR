...
// JwtUtil클래스를 테스트 코드 실행 시 IoC 처리
// (스프링 컨테이너에 강제로 등록 후 테스트 환경에 사용할 수 있게 함)
...
public class JwtUtilTest {
    ...
    // @Test가 붙은 메서드는 톰캣 서버를 켜지 않아도 뒤에
    // 숨어있는 테스트 엔진이 알아서 실행시킬 수 있도록 처리한다.

    @Test
    public void jwtUtil_test() {


        // Given : 테스트 하기 위한  상태를 코드로 작성
        String bearerJwt = JwtUtil.create(user);

        //When : 우리가 진짜로 검증하고 싶은 핵심 동작을 코드로 작성
...
//        System.out.println("토큰에서 추출한 rolse : " + decodedToken.getRoles());
