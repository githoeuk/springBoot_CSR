...
public class SwaggerConfig {
    ...
    private static final String JWT_SCHEMA_NAME = "jwtAuth";

    @Bean
    public OpenAPI openAPI() {
        // 1. 문서 상단에 노출될 기본 정보
        Info info = new Info()
                .title("블로그 REAST API 명세")
                .version("v1.0.0")
                .description("Spring boot 기반에 JWT 기반 프로젝트");
        // 2. 전역 보안 요구 사항 : 모든 API JWT 인증 적용
        SecurityRequirement securityRequirement = new SecurityRequirement()
                .addList(JWT_SCHEMA_NAME);
        // 3. 보안 스키마 정의 (HTTP Bearer 방식, 포멧은 JWT)
        // 이 설정 있어야 Swagger UI 측 상단에 인증 버튼이 생성 됨
